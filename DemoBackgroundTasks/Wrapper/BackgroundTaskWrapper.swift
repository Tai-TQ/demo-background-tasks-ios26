...
@objcMembers
public final class BackgroundTaskWrapper: NSObject {
    public static let shared = BackgroundTaskWrapper()
    private var runningTaskIds: [String: BGContinuedProcessingTask?] = [:]
    ...
    public func setup() {
        NSLog("Setup BackgroundTaskWrapper successfully")
    }
    ...
    ///
    /// - Important: Must be called while the app is in the **foreground** (active state).
    ...
    ///
    /// - Throws:
    ///   - `BGTaskError.notOnMainThread` if not called from the main thread.
...
//                        requestGPU: Bool = false,
                        ...
                        expirationHandler: ((String) -> Void)?) throws -> String
    {
        ...
        let bundleId = Bundle.main.bundleIdentifier ?? "vn.com.vng.zingalo"
...
//        }
    ...
    ///
    /// - Note: Only available on iOS 26.0+. Calling on earlier versions throws `.unsupportedOSVersion`.
    ///
    /// - Parameters:
    ...
    ///
    /// - Throws:
    ///   - `BGTaskError.unsupportedOSVersion` on iOS < 26.0.
    ...
    public func updateProgress(for taskId: String,
        ...
        guard #available(iOS 26.0, *) else {
            throw BGTaskError.unsupportedOSVersion
        }
...
extension BackgroundTaskWrapper {
    ...
    private func removeAllPendingTask() {
        lock.withLock { runningTaskIds = runningTaskIds.compactMapValues { $0 } }
    ...
    private func snapshotRunningTaskIds() -> [String: BGContinuedProcessingTask?] {
